...
def raise_buffers(path, minimum):
    """Return (panes_changed, total_panes) after rewriting path in place."""
    d = bytearray(open(path, "rb").read())
    off, changed, total = 0x10, 0, 0
    while off + 8 <= len(d):
        ...
        if magic == b"txt1":
            total += 1
            buf_off = off + 0x4C
            buf = struct.unpack(">H", d[buf_off:buf_off + 2])[0]
            if buf < minimum:
...
grand = 0
for f in targets:
    changed, total = raise_buffers(os.path.join(blyt_dir, f), minimum)
    grand += changed
    print(f"{f:<18} {changed}/{total} text panes raised to {minimum} bytes")
